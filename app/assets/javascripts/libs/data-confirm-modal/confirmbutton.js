/**
   https://github.com/dealfonso/jsconfirm-buttons
        ...
        return createTag(".modal.fade", { tabindex : "-1", role : "dialog", "aria-hidden" : "true", "data-keyboard": "true"  })._append(
            createTag(".modal-dialog.modal-dialog-centered", { role : "document" })._append(
                createTag(".modal-content")._append(
                    ...
                    createTag(".modal-footer.justify-content-between.flex-row-reverse")._append(
...
titletxt                        }
    ...
    function addconfirmation(els, options = {}) {
        ...
        els.forEach(function(el) {
            ...
            el.addEventListener('click', function(e) {
                ...
                // If there is a text target and a text, put it there
                if ((settings.content !== null) && (settings.texttarget !== null)) {
                    let texttarget = dialog.querySelector(settings.texttarget);
                    if (texttarget !== null) {
                        texttarget.textContent = settings.content;
                    }
                } else {
                    // If there is a text target and a text, put it there
                    if (settings.contenthtml !== null) {
                        let texttarget = dialog.querySelector(settings.texttarget);
                        if (texttarget !== null) {
                            texttarget.innerHTML = settings.contenthtml;
                        }
                    }
                }
                ...
                if ((settings.titletxt !== null) && (settings.titletarget !== null)) {
                    ...
                    if (settings.titletxt === 'Selecione a cor da pulseira do paciente') {
                        dialog.querySelector('.modal-header').classList.add('modal-title-dark');
                        dialog.querySelector('.modal-body').classList.add('p-0');
                        dialog.querySelector('.modal-body').classList.add('pt-2');
                    }
                }
    ...
    function init(document) {
        ...
        //     initialize(el)
        // });
        document.querySelectorAll("[verify]").forEach(function(el) {
            initialize_verification(el)
        });
